...
it('AuthService registers a user', async () => {
  ...
  const userToRegister = {firstName: "Ryan", lastName: "Cyrus", email: "ryan.cyrus@live.com"};
...
it.skip('AuthService logs in a user', async () => {
  ...
  const userToRegister = {firstName: "Ryan", lastName: "Cyrus", email: "ryan.cyrus@live.com"};
  const user = await authService.registerUser(userToRegister, "password");
  await authService.verifyLoginFunc(userToRegister.email, 'password', (err: any, data: any, msg: any) => {
    expect(err).toBeNull();
